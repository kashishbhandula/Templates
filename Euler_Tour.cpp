...
void Recursion(in node, vector<vector<in>> &graph, vector<in> &ht, vector<in> &vis, vector<in> &subtree,vector<in>&Euler, in parent = 0)
{
    vis[node] = 0;
    ht[node] = 1 + ht[parent];
    ...
    for (auto it : graph[node])
    {
        if (vis[it])
        {
            Recursion(it, graph, ht, vis, subtree,Euler,node);
            Euler.push_back(node);
        }
        if (parent != it)
        {
...
void solve()
{
    ...
    vector<in> vis(n + 1, 1);
    vector<in> Euler;
    Recursion(1, graph, ht, vis, subtree,Euler);
    debug(Euler);
