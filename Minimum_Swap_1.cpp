...
in Minimum_Swap(vector<in> &a, vector<in> &b)
{
    in cnt = 0;
    in n=a.size();
    vector<vector<in>> hash(1000007);
    ...
    repf(i, 0, n)
    {
        ...
        {
            cnt++;
            in idx = *hash[b[i]].begin();
            hash[b[i]].erase(hash[b[i]].begin());
            hash[a[i]].erase(hash[a[i]].begin());
            hash[a[i]].push_back(idx);
            swap(a[i], a[idx]);
